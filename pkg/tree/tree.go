...
// PrintTreeWithContainers prints the entire process tree with container info.
func (pt *ProcessTree) PrintTreeWithContainers(w io.Writer, mgr *containers.Manager) {
	roots := pt.findRoots()
	for _, pid := range roots {
		pt.printNodeWithContainer(w, pid, "", true, true, mgr)
	}
}

// PrintLineagesWithContainers prints merged lineages with container info.
func (pt *ProcessTree) PrintLineagesWithContainers(w io.Writer, pids []int32, mgr *containers.Manager) error {
	if len(pids) == 0 {
		return nil
	}
	...
	subtree.PrintTreeWithContainers(w, mgr)
	return nil
}

// printNodeWithContainer recursively prints a node with container info.
func (pt *ProcessTree) printNodeWithContainer(w io.Writer, pid int32, prefix string, isRoot bool, isLast bool, mgr *containers.Manager) {
	td := pt.nodes[pid]
	if td == nil {
		return
	}
	...
	if isRoot {
		fmt.Fprintf(w, "[%d] %s%s\n", td.Pid, td.Comm, containerLabel)
	} else {
		connector := "├── "
		if isLast {
			connector = "└── "
		}
		fmt.Fprintf(w, "%s%s[%d] %s%s\n", prefix, connector, td.Pid, td.Comm, containerLabel)
	}
	...
	for i, childPid := range children {
		isLastChild := i == len(children)-1
		pt.printNodeWithContainer(w, childPid, newPrefix, false, isLastChild, mgr)
	}
}

// PrintTreeWithContainersColored prints the tree with colors.
func (pt *ProcessTree) PrintTreeWithContainersColored(w io.Writer, mgr *containers.Manager) {
	roots := pt.findRoots()
	for _, pid := range roots {
		pt.printNodeColored(w, pid, "", true, true, mgr)
	}
}

// PrintLineagesWithContainersColored prints merged lineages with colors.
func (pt *ProcessTree) PrintLineagesWithContainersColored(w io.Writer, pids []int32, mgr *containers.Manager) error {
	if len(pids) == 0 {
		return nil
	}
	...
	subtree.PrintTreeWithContainersColored(w, mgr)
	return nil
}

// printNodeColored recursively prints a node with colors.
func (pt *ProcessTree) printNodeColored(w io.Writer, pid int32, prefix string, isRoot bool, isLast bool, mgr *containers.Manager) {
	td := pt.nodes[pid]
	if td == nil {
		return
	}
